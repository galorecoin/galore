...
namespace {
    class CGaloreAddressVisitor : public boost::static_visitor<bool> {
    private:
        CGaloreAddress *addr;
    public:
        CGaloreAddressVisitor(CGaloreAddress *addrIn) : addr(addrIn) { }
...
bool CGaloreAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CGaloreAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CGaloreAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CGaloreAddressVisitor(this), dest);
}

bool CGaloreAddress::IsValid() const {
...
CTxDestination CGaloreAddress::Get() const {
    if (!IsValid())
...
bool CGaloreAddress::GetKeyID(CKeyID &keyID) const {
...
bool CGaloreAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CGaloreSecret::SetKey(const CKey& vchSecret) {
...
CKey CGaloreSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CGaloreSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CGaloreSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CGaloreSecret::SetString(const std::string& strSecret) {
