...
const string strMessageMagic = "Galore Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    else if (nHeight >= 3 && nHeight < 91)    { nSubsidy = 0.1; }

    else if (nHeight >= 91 && nHeight < 1000)    { nSubsidy = 0.01; }

    else if (nHeight >= 1000 && nHeight < 200000)    { nSubsidy = 0.0001; }

    else { nSubsidy = 0.001 * COIN; }
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if(nBestHeight >= 20 && nBestHeight < 1000	)
    	{ nSubsidy = 	1	 * COIN; }
	else if(nBestHeight >= 	1001	 &&  nBestHeight < 	5000	)
		{ nSubsidy = 	3	 * COIN; }
	else if(nBestHeight >= 	5001	 &&  nBestHeight < 	10000	)
		{ nSubsidy = 	5	 * COIN; }
	else if(nBestHeight >= 	10001	 &&  nBestHeight < 	13000	)
		{ nSubsidy = 	10	 * COIN; }
	else if(nBestHeight >= 	13001	 &&  nBestHeight < 	16000	)
		{ nSubsidy = 	12	 * COIN; }
	else if(nBestHeight >= 	16001	 &&  nBestHeight < 	19000	)
		{ nSubsidy = 	13	 * COIN; }
	else if(nBestHeight >= 	19001	 &&  nBestHeight < 	22000	)
		{ nSubsidy = 	14	 * COIN; }
	else if(nBestHeight >= 	22001	 &&  nBestHeight < 	25000	)
		{ nSubsidy = 	15	 * COIN; }
	else if(nBestHeight >= 	25001	 &&  nBestHeight < 	28000	)
		{ nSubsidy = 	16	 * COIN; }
	else if(nBestHeight >= 	28001	 &&  nBestHeight < 	31000	)
		{ nSubsidy = 	17	 * COIN; }
	else if(nBestHeight >= 	31001	 &&  nBestHeight < 	35000	)
		{ nSubsidy = 	18	 * COIN; }
	else if(nBestHeight >= 	35001	 &&  nBestHeight < 	45000	)
		{ nSubsidy = 	19	 * COIN; }
	else if(nBestHeight >= 	45001	 &&  nBestHeight < 	70000	)
		{ nSubsidy = 	21	 * COIN; }
	else if(nBestHeight >= 	70001	 &&  nBestHeight < 	73000	)
		{ nSubsidy = 	18	 * COIN; }
	else if(nBestHeight >= 	73001	 &&  nBestHeight < 	77000	)
		{ nSubsidy = 	12	 * COIN; }
	else if(nBestHeight >= 	77001	 &&  nBestHeight < 	100000	)
		{ nSubsidy = 	8	 * COIN; }
	else if(nBestHeight >= 	100001	 &&  nBestHeight < 	200000	)
		{ nSubsidy = 	4	 * COIN; }
	else if(nBestHeight >= 	200001	 &&  nBestHeight < 	500000	)
		{ nSubsidy = 	1	 * COIN; }
	else if(nBestHeight >= 	500001	 &&  nBestHeight < 	2000000	)
		{ nSubsidy = 	0.5	 * COIN; }
    else if(nBestHeight >= 2000000)
    	{ nSubsidy = 0 * COIN; }        // discuss
    else { nSubsidy = 1 * COIN; }

    return nSubsidy + nFees;
}

static int64_t nTargetTimespan = 3 * 90;  // 4.5 mins
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (vtx.empty())
        return DoS(100, error("CheckBlock() : vtx.empty()=true"));
    if (!vtx[0].IsCoinBase()) {
        LogPrintf("Display vtx[0]=%s\n", vtx[0].ToString().c_str());
        return DoS(100, error(" *****CheckBlock() : first tx (vtx[0]) is not coinbase"));
    }
        ...
        if(MasternodePayments)
        {
            ...
            if(IsProofOfStake() && pindex != NULL){
                if(pindex->GetBlockHash() == hashPrevBlock){
                    ...
                    CGaloreAddress address2(address1);
