...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeGaloreAmount"))
        settings.setValue("nAnonymizeGaloreAmount", 1000);
    nAnonymizeGaloreAmount = settings.value("nAnonymizeGaloreAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeGaloreAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeGaloreAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeGaloreAmount:
            return QVariant(nAnonymizeGaloreAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeGaloreAmount:
            nAnonymizeGaloreAmount = value.toInt();
            settings.setValue("nAnonymizeGaloreAmount", nAnonymizeGaloreAmount);
            emit AnonymizeGaloreAmountChanged(nAnonymizeGaloreAmount);
            break;
        default:
            break;
        }
