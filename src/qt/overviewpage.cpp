...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeGaloreAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeGaloreAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAmountAndRounds = strAnonymizeGaloreAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeGaloreAmount*COIN) nMaxToAnonymize = nAnonymizeGaloreAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeGaloreAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeGaloreAmount));
        strAmountAndRounds = strAnonymizeGaloreAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
